...
async def create_portal_route_func(portal_data: PortalBaseModel):
    try:
        data = await create_portal(portal_data)
        return success_response(data, message=ResponseMessage.OK_CREATE.value, status_code=201)
...
async def update_portal_route_func(slug: str, portal_data: PortalUpdateModel):
    try:
        ...
        data = await update_portal(slug, update_data)
        return success_response(data, message=ResponseMessage.OK_UPDATE.value, status_code=200)
    except Exception as e:
        return error_response(message=str(e), status_code=400)
...
async def delete_portal_route_func(slug: str):
    try:
        data = await delete_portal(slug)
        return success_response(data, message=ResponseMessage.OK_DELETE.value, status_code=200)
    except Exception as e:
        return error_response(message=str(e), status_code=400)

# Get all portals
@router.get("/", response_model=PortalModel, status_code=status.HTTP_200_OK)
async def get_portals_route_func():
    try:
        data = await get_portals()
...
async def get_portal_by_slug_route_func(slug: str):
    try:
        data = await get_portal_by_slug(slug)
